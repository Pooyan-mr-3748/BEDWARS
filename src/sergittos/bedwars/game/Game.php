...
class Game {
    ...
    public function startGameFromLeader(Session $leader): void {
        // بررسی اینکه آیا بازیکن لیدر است یا خیر
        if ($leader->isLeader()) {
            $party = $leader->getParty();  // فرض می‌کنیم Session دارای متدی به نام getParty است
            foreach ($party->getMembers() as $member) {
                if (!$member->isInMatch()) {
                    $this->startMatchForPlayer($member);
                }
            }
            $this->broadcastMessage("The game has started! All party members have joined the match.");
        } else {
            // اگر بازیکن لیدر نباشد، اجازه ندهید که بازی استارت شود
            $leader->sendMessage("You are not the leader and cannot start the game.");
        }
    }
