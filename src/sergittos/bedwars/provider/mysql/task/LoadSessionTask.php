...
class LoadSessionTask extends MysqlAsyncTask {
    ...
    private function insertIfNotExists(mysqli $mysqli): void {
        $stmt = $mysqli->prepare("INSERT IGNORE INTO bedwars_users (xuid, coins, kills, wins, levels, deaths, final_kills, xp, flying_speed, auto_teleport, night_vision) VALUES (?, 0, 0, 0, 1, 0, 0, 0, 1, true, true)");
    ...
    public function onCompletion(): void {
        ...
        $session->setLevel((float) $data["levels"]);
        $session->setDeaths((int) $data["deaths"]);
        $session->setFinalKills((int) $data["final_kills"]);
        $session->setxp((int) $data["xp"]);
